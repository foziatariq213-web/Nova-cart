...
.product-info .price-row .old-price {
    font-size: 14px;
    color: #6b7280;
    text-decoration: line-through;
}

.product-info .price-row .discount {
    font-size: 12px;
    font-weight: 600;
    color: #34d399;
...
<div class="gift-results-bg py-12">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div>
                    <div class="breadcrumb-custom mb-2">
                        <a href="{{ route('gift.finder') }}">← Back to Gift Finder</a>
                    </div>
                    <h2 class="text-3xl font-black text-white">
                        🎁 Recommended Gifts
                    </h2>
                    <p class="text-gray-400 text-sm mt-1">
                        Found <span class="text-purple-400 font-bold">{{ $products->total() }}</span> gifts matching your preferences
                    </p>
                </div>
    ...
    @endif
                        ...
                        <div class="product-info">
                            <div class="flex items-center justify-between">
                                <span class="category-tag">{{ $product->category->name ?? 'Gift' }}</span>
                                @if($product->mood)
                                    <span style="font-size:14px;">{{ $product->mood->icon ?? '😊' }}</span>
                                @endif
                            </div>
                            ...
                            <div class="description">
                                {{ \Illuminate\Support\Str::limit($product->description ?? 'Perfect gift for your loved ones', 60) }}
                            </div>

                            <div class="price-row">
                                <span class="current-price">Rs. {{ number_format($product->new_price, 0) }}</span>
                                @if($product->old_price && $product->old_price > $product->new_price)
                                    <span class="old-price">Rs. {{ number_format($product->old_price, 0) }}</span>
                                    <span class="discount">-{{ round((($product->old_price - $product->new_price) / $product->old_price) * 100) }}%</span>
                                @endif
                            </div>
                            ...
                            <div class="mood-tags">
                                @if($product->gift_for)
                                    <span class="tag">🎯 {{ $product->gift_for }}</span>
                                @endif
                                @if($product->occasion)
                                    <span class="tag">🎉 {{ $product->occasion }}</span>
                                @endif
                            </div>
        ...
        @else
            ...
            <div class="no-results">
                ...
                <p>We couldn't find any gifts matching your preferences. Try adjusting your selections.</p>
                <a href="{{ route('gift.finder') }}" class="btn-back">
                    <i class="fa-solid fa-arrow-left"></i> Back to Gift Finder
                </a>
            </div>
...
function addToCart(productId, button) {
    ...
    // Using url() helper instead of route() to avoid missing parameter error
...
// ===================== CONFETTI ON RESULTS =====================
(function() {
    // Small confetti burst when page loads
    const colors = ['#7c3aed', '#ec4899', '#6366f1', '#f59e0b', '#34d399'];
    for (let i = 0; i < 30; i++) {
        setTimeout(() => {
            ...
            el.style.cssText = `
                ...
                border-radius: 2px;
                opacity: 0.8;
                animation: confetti-fall ${0.8 + Math.random() * 0.8}s linear forwards;
            `;
    ...
    if (!document.getElementById('confetti-style')) {
        ...
        style.textContent = `
            @keyframes confetti-fall {
                to {
                    transform: translateY(${window.innerHeight + 50}px) rotate(540deg);
                    opacity: 0;
                }
            }
        `;
